"""
LNO vs LRN-FNO Comparison Demo - 2D Navier-Stokes
...
def compare_navier_stokes(epochs=150):
    """Compare FNO, LRN-FNO, and LNO on 2D Navier-Stokes."""
    print("=" * 70)
    print(f"LNO vs LRN-FNO Comparison - 2D Navier-Stokes (Epochs: {epochs})")
    print("=" * 70)
    ...
    # 1. Prepare Data
    print("\nPreparing 2D Navier-Stokes Dataset...")
    train_dataset = NavierStokesDataset(resolution=64, num_samples=200, train=True)
    test_dataset = NavierStokesDataset(resolution=64, num_samples=50, train=False)
    ...
    sample_f, sample_u = train_dataset[0]
    in_channels = sample_f.shape[0]
    out_channels = sample_u.shape[0]
    ...
    trainer = LRNTrainerV2(
        ...
        stage2_lr=1e-4,
        device=str(device),
        checkpoint_dir='lno_checkpoints/lrn_ns'
    )
    trainer.train()
    ...
    # Paper config for NS2d: 8 layers, 256 dim
    lno = LNO2d(
        in_channels=in_channels,
        out_channels=out_channels,
        embed_dim=256,
    ...
    # 5. Summary
    print("\n" + "=" * 70)
    print("COMPARISON SUMMARY - 2D Navier-Stokes")
    print("=" * 70)
    ...
    # Save results
    save_results(results, 'navier_stokes')
...
def visualize_comparison(fno, lrn, lno, dataset, device, results):
    ...
    with torch.no_grad():
        pred_fno = fno(f_input).squeeze(0)
        pred_lrn = lrn(f_input, return_latents=False)['prediction'].squeeze(0)
        pred_lno = lno(f_input).squeeze(0)
    
    # Visualize first channel (vorticity at final time step)
    channel = -1  # Last time step
    
    fig, axes = plt.subplots(4, 3, figsize=(15, 16))
    
    # Row 1: Input and Ground Truth
    im0 = axes[0, 0].imshow(f[0].cpu().numpy(), cmap='RdBu_r')
    axes[0, 0].set_title('Input: ω(t=0)')
    plt.colorbar(im0, ax=axes[0, 0])
    
    im1 = axes[0, 1].imshow(u_gt[channel].cpu().numpy(), cmap='RdBu_r')
    axes[0, 1].set_title(f'Ground Truth: ω(t=T)')
    plt.colorbar(im1, ax=axes[0, 1])
    
    axes[0, 2].axis('off')
    axes[0, 2].text(0.5, 0.5, '2D Navier-Stokes\nVorticity Evolution', 
                    ha='center', va='center', fontsize=14, transform=axes[0, 2].transAxes)
    
    # Row 2: FNO
    im3 = axes[1, 0].imshow(pred_fno[channel].cpu().numpy(), cmap='RdBu_r')
    axes[1, 0].set_title(f"FNO (RelL2: {results['fno']['error']:.4f})")
    plt.colorbar(im3, ax=axes[1, 0])
    
    err_fno = np.abs(u_gt[channel].cpu().numpy() - pred_fno[channel].cpu().numpy())
    ...
    # Row 3: LRN-FNO
    im5 = axes[2, 0].imshow(pred_lrn[channel].cpu().numpy(), cmap='RdBu_r')
    axes[2, 0].set_title(f"LRN-FNO (RelL2: {results['lrn']['error']:.4f})")
    plt.colorbar(im5, ax=axes[2, 0])
    
    err_lrn = np.abs(u_gt[channel].cpu().numpy() - pred_lrn[channel].cpu().numpy())
    ...
    # Row 4: LNO
    im7 = axes[3, 0].imshow(pred_lno[channel].cpu().numpy(), cmap='RdBu_r')
    axes[3, 0].set_title(f"LNO (RelL2: {results['lno']['error']:.4f})")
    plt.colorbar(im7, ax=axes[3, 0])
    
    err_lno = np.abs(u_gt[channel].cpu().numpy() - pred_lno[channel].cpu().numpy())
    ...
    plt.tight_layout()
    plt.savefig('../../results/plots/lno_vs_lrn_navier_stokes.png', dpi=150)
    print("\nVisualization saved to results/plots/lno_vs_lrn_navier_stokes.png")
    plt.close()
...
if __name__ == '__main__':
    ...
    compare_navier_stokes(epochs=args.epochs)
