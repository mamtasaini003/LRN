"""
Ablation Study: Optimal Training Split for LRR-FNO (Navier-Stokes).
...
def train_fno(args, device, train_loader, test_loader):
    ...
    model = FNO2d(
        ...
        modes1=args.modes, modes2=args.modes, width=args.width, num_layers=4
    ).to(device)
...
def load_data(args):
    """Load Navier-Stokes dataset with synthetic fallback."""
...
def run_ablation(args):
    ...
    for stage1, stage2 in splits:
        ...
        loss_fn = LRNLoss(temperature=0.07, lambda_mse=5.0, lambda_nce=0.001, symmetric_nce=True)
        ...
        trainer = Trainer(
            ...
            stage1_lr=1e-3, stage2_lr=1e-4, checkpoint_dir=f'checkpoints_ablation_ns_{stage1}_{stage2}'
        )
        trainer.train()
    ...
    # Save results
    out_dir = Path('results/ablation/navier_stokes')
