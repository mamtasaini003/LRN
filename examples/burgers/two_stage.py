"""
Latent Rescaling Network (LRN) - 1D Burgers Experiment (Two-Stage).
...
def train_lrr(args, device, train_loader, test_loader):
    ...
    trainer = Trainer(
        ...
        weight_decay=1e-4,
        checkpoint_dir='checkpoints_burgers_twostage',
        scheduler_type='step',
...
def plot_comparison(fno_model, lrr_model, loader, device, metrics, out_dir):
    ...
    for idx, title, fname in zip(indices, titles, filenames):
        ...
        plt.plot(y_lrr, 'g-.', label='LRR (Ours)', alpha=0.9, linewidth=2)
...
def main():
    ...
    # Save Results
    out_dir = Path('results/plots/burgers/two_stage')
