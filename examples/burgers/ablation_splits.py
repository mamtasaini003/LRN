"""
Ablation Study: Optimal Training Split for LRR-FNO.
...
def train_fno(args, device, train_loader, test_loader):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x, y in train_loader:
            ...
            optimizer.zero_grad()
            out = model(x)
            if out.ndim == 3 and out.shape[-1] == 1: out = out.permute(0, 2, 1)
...
def load_data(args):
    """Load Burgers dataset with synthetic fallback."""
    logger.info("Loading Burgers dataset...")
    try:
        ...
        train_dataset = BurgersDataset(num_samples=args.n_train, resolution=128)
        test_dataset = BurgersDataset(num_samples=args.n_test, resolution=128)
        return (
...
def run_ablation(args):
    ...
    for stage1, stage2 in splits:
        ...
        loss_fn = LRNLoss(temperature=0.07, lambda_mse=5.0, lambda_nce=0.001)
        ...
        trainer = Trainer(
            ...
            checkpoint_dir=f'checkpoints_ablation_{stage1}_{stage2}',
            scheduler_type='step',
    ...
    # Save results
    out_dir = Path('results/ablation/burgers') # More specific path
    ...
    with open(out_dir / 'metrics.json', 'w') as f: # standard name
    ...
    plt.plot(ratios, errors, 'o-', label='LRR-FNO (Ours)', linewidth=2, color='green')
    plt.axhline(y=fno_err, color='red', linestyle='--', label='Baseline FNO')
